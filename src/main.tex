...
\begin{document}
\title{A {\color{red}{Truly}} Fast and Simple Algorithm for Submodular Maximization with a Knapsack Constraint}
\author[1]{}
...
In this paper we consider the problem of maximizing a monotone\footnote{
...
They presented an algorithm achieving an approximation of $(1-\nicefrac[]{1}{e}-\varepsilon)$ whose running time equals $O(n^2(\varepsilon ^{-1}\log n)^{\text{poly}(\varepsilon^{-1})})$, for every constant $\varepsilon >0$.
...
Building upon the approach of \cite{badanidiyuru2014fast}, Ene and Nguy{\~{\^{e}}}n \cite{Alina2017} presented an algorithm achieving the same approximation guarantee whose running time is $O(\varepsilon^{-O(\varepsilon^{-4})}n \log^2 n)$.
...
{\color{red}{
Unfortunately, both algorithms \cite{badanidiyuru2014fast,Alina2017} are not simple nor fast, as even stated by the authors themselves, {\em e.g.}, see \cite{Alina2017}.
To best exemplify the impracticality of these algorithms one needs only to choose $\varepsilon = \nicefrac[]{1}{4}$.
This results in an approximation of only $(1-\nicefrac[]{1}{e}-\nicefrac[]{1}{4})$, which is worse than the fast algorithm of \cite{khuller1999budgeted} since $(1-\nicefrac[]{1}{e}-\nicefrac[]{1}{4})<(1-e^{-\nicefrac[]{1}{2}})$.
Moreover, the running time of the faster of these two algorithms is at least $2^{512} n$ \cite{Alina2017}.
%Thus, the two algorithm of \cite{badanidiyuru2014fast,Alina2017} are far from being simple and fast.  
Thus, there is a need for {\em truly} fast and simple algorithms for the \SK problem. %with a provable improved approximation guarantee. 
...
%Another line of research examines the adaptive complexity of submodular maximization, that is the goal is to design approximation algorithms that are faster if (polynomial) parallelization is allowed.
%Balkanski and Singer developed a $1/3$-approximation algorithm for monotone submodular maximization with cardinality constraint that needs only $O(\log n)$ sequential rounds.
%They also showed that no algorithm can achieve a better approximation than $O(\frac{1}{\log n})$ with fewer than $O(\frac{\log n}{\log\log n})$ \cite{balkanski2018adaptive}.
%Three independent works design an algorithm with approximation ratio of $1 - \frac{1}{e} - \varepsilon$ and $O(\log n)$ sequential rounds \cite{fahrbach2019submodular, balkanski2019exponential, ene2019submodular}.


\paragraph*{Our Results}
Our results can be partitioned into two parts: $(1)$ {\color{red}{truly}} fast and simple combinatorial algorithms for \SK; and $(2)$ a general method for ``amplifying'' any algorithm for \SK by improving its approximation factor while losing little in the running time.
Let us elaborate on the above.

\vspace{3pt}
\noindent {\bf{{\color{red}{Truly}} Fast and Simple Combinatorial Algorithms:}} We note that the proof that the fast algorithm of \cite{khuller1999budgeted} (named Modified Greedy by the authors) achieves an approximation guarantee of $(1-e^{-\nicefrac[]{1}{2}})$ is incorrect \cite{naor}.
...
{\color{red}{Moreover, the algorithm requires only $3n^2/2+n$ value oracle queries.}}
\end{theorem}
...
Specifically, given any black box algorithm $\mathcal{A}$ that obtains an approximation guarantee of $r$ and a number $k\in \mathbb{N}$ of executions, we show how to obtain a new {\color{red}{simple}} algorithm for \SK with a better approximation than $r$.
The running time of the new algorithm equals $k$ times the running time of the black box algorithm plus an additional $3n^2/2+n$ oracle queries.
This is summarized in the following theorem.
\begin{theorem}
...
For example, choosing the black box algorithm $\mathcal{A}$ to be the algorithm whose existence is guaranteed by Theorem \ref{thrm:ModifiedSquared} ({\color{red}{we note that the approximation of $0.453647$ guaranteed in Theorem \ref{thrm:ModifiedSquared} is obtained {\em without} any amplification}}), amplifying it with $k=6$ we can obtain an algorithm achieving an improved approximation of $0.48$ and only uses  $10.5n^2+7n$ oracle queries.
...
{\color{red}{We note that the amplified algorithm, whose existence is guaranteed by Theorem \ref{thrm:Amplification}, is remarkably simple though its proof requires some work.
We believe our amplification framework might be of independent interest.
}}
...
The crucial point in the analysis is that the algorithm needs to find the {\em exact} subset of the most valuable elements in the optimal solution.
...
case of \SK with uniform costs, {\em i.e.}, a cardinality constraint, along with a matching
lower bound in the oracle model is known since the late 70's and is due to the
works of Nemhauser, Wolsey and Fisher \cite{NW78,Nemhauser1978}.
Their works also achieved an approximation of $\nicefrac[]{1}{2}$ for the problem
of maximizing a monotone submodular function subject to a matroid
constraint \cite{FNW78}.
An optimal approximation of $(1-e^{-1})$ for the latter problem was attained with
the introduction of the continuous greedy and pipage rounding algorithms by Calinescu {\em et al.} \cite{CCPV11}.
The \SK problem can also be generalized to the case of {\em multiple} knapsack constraints, for which Kulik {\em et al.} provided a tight approximation of $(1-e^{-1}-\epsilon)$ for every $\epsilon>0$ \cite{KST13}.

{\color{red}{Motivated by the need for scalable algorithms, another line of research examines the adaptive complexity of submodular maximization (the design of approximation algorithms in the presence of parallelization).
First introduced by Balkanski and Singer \cite{balkanski2018adaptive}, an approximation of $\nicefrac[]{1}{3}$ was given for maximizing a monotone submodular function with a cardinality constraint in $O(\log{n})$ rounds.
This was subsequently improved independently by \cite{fahrbach2019submodular, balkanski2019exponential, ene2019submodular} to an almost tight approximation of $1 - \frac{1}{e} - \varepsilon$ while retaining the number of sequential rounds.
Recently, parallel algorithm with bounded adaptivity were given for the more general maroid constraint \cite{BRS18,CQ18,ENV18} and for unconstrained submodular maximization \cite{CFK18}.
}}
%Another line of research examines the adaptive complexity of submodular maximization, that is the goal is to design approximation algorithms that are faster if (polynomial) parallelization is allowed.
%Balkanski and Singer developed a $1/3$-approximation algorithm for monotone submodular maximization with cardinality constraint that needs only $O(\log n)$ sequential rounds.
%They also showed that no algorithm can achieve a better approximation than $O(\frac{1}{\log n})$ with fewer than $O(\frac{\log n}{\log\log n})$ \cite{balkanski2018adaptive}.
...
Moreover, for an ordered set $A = \{a_1, \dots, a_k\}$ we use $A_i$ to denote its prefix of size $i$, {\em i.e.}, $\{a_1, \dots, a_i\}$ ($A_0$ is set to $\emptyset$).
...
We are now ready to prove Theorem \ref{thrm:CorrectModifiedGreedy}.
...
\begin{proof}[Proof of Theorem \ref{thrm:CorrectModifiedGreedy}]
	...
	Consider two cases depending on how many elements of $O$ are dropped by the greedy algorithm, {\em i.e.}, $|O \setminus S|$.
...
Otherwise, $|O \setminus S| \geq 2$.
	Denote by $a$ and $b$ the first and second elements of $O$ the greedy algorithm drops, {\em i.e.} $a$ is the considered element at the first time the condition in line~\ref{line:dropped} of Algorithm~\ref{alg:greedy} is evaluated to false {\color{red}{for an element of $O$}} and $b$ is the considered element the second time the same condition is evaluated to false {\color{red}{for an element of $O$}}.
	...
	To prove the theorem we {\color{red}{require bounds on both size and value}}.
...
{\color{red}{Inequalities (\ref{mgreedy:ineq1}) and (\ref{mgreedy:ineq3}), along with the fact that $c(A\cup B\cup \{ b\})>c(O)$, provide a lower bound on the size of $B$:}}
	\begin{equation}
		\label{mgreedy:ineq4}
		c(B) > (2\delta)c(O).
	\end{equation}
...
Our algorithm returns the best between the greedy algorithm (Algorithm \ref{alg:greedy}) and highest value pair of elements ({\color{red}{the algorithm also implicitly considers singletons and thus explores a superset of Algorithm's \ref{alg:mgreedy} search space}}).
...
{\color{red}{In this section we present our amplification framework.
Specifically, we show a simple algorithm, which given
an $r$-approximation algorithm $\mA$ for \SK (where $r<1/2$), can
use $\mA$ to derive an $r'$-approximation for
\SK (where $r<r' <1/2$) using  a constant number of calls
to $\mA$ and $3n^2/2+n$ additional oracle queries.
}}
...
$\nicefrac[]{f(\{a,b\})}{f(\pVal)} \geq \rho$ into {\em buckets}, where the subsets
in each bucket have the same value up to a factor of $(1+\epsilon)$.
...
We remark that while the proofs of our claims regarding the amplification framework are {\color{red}{involved and require some work}}, the resulting amplified algorithm is simple and comes down to {\color{red}{meticulously}} choosing a constant number of pairs of elements and extending {\color{red}{each of}} them to a complete solution using the given (unamplified) algorithm {\color{red}{$\mA$}} as a black-box.
...
The following functions, which already appeared in Theorem \ref{thrm:Amplification}, are useful throughout the analysis of the algorithm:
...
$$ A(\alpha) = \frac{1}{1-e^{-\alpha}}-\frac{1}{B(\alpha)}$$
 ...
 {\color{red}{
 	Let $\mA$ be a $r$-approximation for $\SK$ where $r<\nicefrac[]{1}{2}$.
 	For any $0< \alpha \leq  \ln 2$, input $U,f,c,\beta$ of $\SK$,
 	and $\epsilon$ such that $0<\epsilon\leq \nicefrac[]{(1-2r)}{r}$, executing
 	Algorithm \ref{algorithm:amplify}  with parameters
 	$(\mA, U, f, c, \beta, \epsilon, A(\alpha))$ returns a
...
%	returns a solution of value at least $(1-e^{-\alpha})f(O)$ and the Lemma holds. Hence, we can assume  $L> 1-\alpha$.
	...
	Hence, $f(G)\geq \nicefrac[]{f(O)}{2}$ or $f(\pVal)\geq \nicefrac[]{f(O)}{2}$
	and thus the lemma holds (note that $\alpha \leq \ln 2$ and therefore
	$1-e^{-\alpha} < \nicefrac[]{1}{2}$). Thus, we can assume that
	$O\setminus \pLarge \nsubseteq G$.
...
\begin{proof}
	...
	Let $\beta M$ be the third highest cost of an element in  $O$.
	As the highest cost of  element in $O$ is $\beta L$,
	using a simple argument we get, $M\leq M(L)$ where $M(L)= \min \{\frac{1-L}{2}, L\}$. Since
	$O\setminus \pLarge \nsubseteq G$, there must be an element from $O \setminus \pLarge$ the greedy algorithm in line \ref{amplify:greedy} drops. Since all the elements
...
%\end{array}
%	\end{equation*}
{\color{red}{This concludes the proof of Corollary \ref{corollary:cor1}.}}
	\end{proof}
...
{\color{red}{This concludes the proof of Corollary \ref{corollary:cor2}.}}
		\end{proof}
...
\begin{proof}
		...
		that $\nicefrac[]{f(\pLarge)}{f(\pVal)} < A(\alpha)(1+\epsilon) ^{i+1}$, as $f(\pLarge) \geq A(\alpha) f(\pVal)$ we get $i\geq 0$.
...
{\color{red}{This concludes the proof of Corollary \ref{corollary:cor3}.}}
	\end{proof}
{\color{red}{We are now ready to finish the proof of Lemma \ref{lemma:amplification}.}}
...
Algorithm \ref{algorithm:amplify} uses $\floor{\log_{1+\epsilon} \left(\nicefrac[]{1}{\rho}\right)}+1$
...
\begin{proof}[Proof of Theorem \ref{thrm:Amplification}]
	...
	= k \end{align*}
	and the number of addition oracle queries is $\frac{3n^2}{2} + n $
	as required.	
\end{proof}
